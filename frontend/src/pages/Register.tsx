...
const Register = () => {
	...
	const { register, confirmCode } = useAuth(); // расширить контекст
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			await register(name, email, password); // сервер создаст пользователя, отправит код
			setStep("confirm"); // переходим к подтверждению
		} catch (err) {
	...
	const handleConfirm = async (e: React.FormEvent) => {
		...
		try {
			await confirmCode(email, code); // сервер проверит код
